...
router.delete('/:id', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (remainingBudget > 0.01 && ad.advertiser?.stellar_public_key && process.env.Shekael_DEV_WALLET_SECRET) {
            try {
                const stellarService = await import('../services/stellar.service.js');
                await stellarService.sendPayment({
                    fromSecretKey: process.env.Shekael_DEV_WALLET_SECRET, // Sale de la wallet de sistema
                    toPublicKey: ad.advertiser.stellar_public_key,
                    amount: remainingBudget.toFixed(7),
                    assetCode: 'MXNe',
                    memo: 'Reembolso Ad Shekael'
                });
