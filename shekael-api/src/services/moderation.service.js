...
async function callGroq(apiKey, content, type) {
    const prompt = `Eres un moderador experto para la red social "Shekael". 
...
async function callGroqVision(apiKey, imageUrl, type) {
    const prompt = `Analiza esta ${type === 'video' ? 'miniatura de video' : 'imagen'} para la red social "Shekael". 
