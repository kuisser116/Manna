...
const JWT_SECRET = process.env.JWT_SECRET || 'Shekael-secret-key-123';
...
function calculateShares(costPerView, context, contentType = 'standard', status = 'completed') {
    ...
    if (!context || context === 'feed') {
        // Sin creador (feed general): 0/20/40/40 (viewer/Shekael/fondo regional)
        baseShares = { creator: 0, viewer: costPerView * 0.20, dev: costPerView * 0.40, barn: costPerView * 0.40 };
    } else {
        // Con creador: 50/20/15/15 (creador/viewer/Shekael/fondo regional)
...
export async function triggerDistribution(userId, adId, postId, viewSeconds, proofToken, context = 'feed', creatorId = null, status = 'completed') {
    ...
    if (!isFallbackAd) {
        try {
            ...
            const DEV_WALLET = process.env.Shekael_DEV_WALLET;
            const DEV_SECRET = process.env.Shekael_DEV_WALLET_SECRET;
            const BARN_WALLET = process.env.Shekael_BARN_WALLET;
                ...
                try {
                    ...
                    if (shares.creator > 0.1 && resolvedCreatorId) {
                        ...
                        if (creator?.stellar_public_key && creator.stellar_public_key !== DEV_WALLET) {
                            stellarService.sendPayment({ fromSecretKey: effectiveSecret, toPublicKey: creator.stellar_public_key, amount: shares.creator.toFixed(7), assetCode: 'MXNe', memo: 'Shekael:ad:creator' }).catch(() => {});
                        }
                    }
                } catch (pErr) {
...
const FALLBACK_ADS = [
    {
        id: 'fallback-Shekael-1',
        title: 'Shekael Network',
        description: 'La plataforma donde tu contenido vale oro. Únete a la revolución del contenido monetizado.',
        media_url: 'https://via.placeholder.com/800x450/1a1a1a/ffffff?text=Shekael+Network',
        media_type: 'banner',
        advertiser_name: 'Shekael Network',
        cta_label: 'Conoce más',
        cta_url: 'https://Shekael.network',
        promo_text: 'Bono de bienvenida',
        promo_code: 'Shekael2024',
        cpm: 1.0,
        budget_usdc: 1000,
        target_audience: 'all',
        status: 'active'
    },
    {
        ...
        cta_url: 'https://Shekael.network/create',
    ...
    },
    {
        ...
        advertiser_name: 'Shekael Network',
        cta_label: 'Explorar',
        cta_url: 'https://Shekael.network/feed',
...
export async function createLocalAd({ advertiserId, community_id, content, budget_usdc, title, media_url, media_type }) {
    const supabase = getDB();
    const adId = uuidv4();
    const ESCROW_WALLET = process.env.Shekael_DEV_WALLET;
