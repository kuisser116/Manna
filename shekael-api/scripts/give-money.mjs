...
async function run() {
    ...
    void(`✅ Archivo .env actualizado exitosamente con el nuevo MXNE_ISSUER.`);
    ...
    for (const user of users) {
        ...
        try {
            ...
            void(`   Creando Trustline para MXNe...`);
            ...
            void(`   ✅ 500 MXNe depositados con éxito a ${user.display_name}.`);
        } catch (err) {
    ...
    void('\n✅ Proceso terminado.');
    void('⚠️ IMPORTANTE: DEBES REINICIAR EL SERVIDOR DE NODE (Ctrl+C y volver a iniciar) PARA QUE EL BACKEND LEA EL NUEVO ISSUER DEL .ENV');
}
