...
export default function StickerPicker({ onSelect, onClose }) {
  ...
  const mine = stickers.filter(s => !s.is_default && s.is_favorite);
  const allMine = stickers.filter(s => !s.is_default);

  const list = tab === 'default' ? defaults : tab === 'fav' ? mine : allMine;
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.picker} onClick={e => e.stopPropagation()}>
        <div className={styles.header}>
          ...
          <label className={styles.uploadBtn} title="Subir sticker">
        ...
        <div className={styles.tabs}>
          <button className={`${styles.tab} ${tab === 'default' ? styles.activeTab : ''}`} onClick={() => setTab('default')}>Default</button>
          <button className={`${styles.tab} ${tab === 'all' ? styles.activeTab : ''}`} onClick={() => setTab('all')}>Mis stickers</button>
        ...
        <div className={styles.grid}>
          ...
          {list.length === 0 && <p className={styles.empty}>Sin stickers</p>}
        </div>
      </div>
    </div>
  );
