...
export function PostCard({ post, isDetail = false }) {
    ...
    const handleLike = async (e) => {
        ...
        try {
            ...
            window.dispatchEvent(new CustomEvent('Shekael:quest-refresh'));

            // Si la misiÃ³n se completa gracias a este like, disparamos el evento de celebraciÃ³n
            if (!wasLiked && res?.data?.missionCompleted) {
                window.dispatchEvent(new CustomEvent('Shekael:celebration'));
            }
        } catch (error) {
        ...
        if (navigator.share) {
            try {
                await navigator.share({
                    title: `Post de ${display_name || 'Shekael'}`,
                    text: 'Mira este aporte en Shekael Network ðŸŒ¾',
                    url: postUrl,
                });
            } catch (err) {
    ...
    const handleCardClick = (e) => {
        // Si el modal de reporte estÃ¡ abierto, ignorar clics en la tarjeta
    ...
    const renderContent = () => {
        ...
        if (isImage) {
            // Nuevo formato HÃ­brido: r2_url|||cid|||caption
            ...
            if (parts[0].startsWith('http') || parts[0].startsWith('r2://')) {
                // Para las demos con r2:// usamos un placeholder o la URL pÃºblica si existe
        ...
        >
            {isCapsule && (
                <div className={styles.capsuleLabel}>
                    <span>âœ¨ CÃ¡psula de Shekael</span>
                </div>
            )}
