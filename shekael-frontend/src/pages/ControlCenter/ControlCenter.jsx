...
export default function ControlCenter() {
    ...
    const getDisplayContent = (post) => {
        ...
        if (post.type === 'image') return parts[2] || 'Sin descripciÃ³n';
        if (post.type === 'video') return post.video_title || parts[1] || 'Sin tÃ­tulo';
    ...
    if (!user?.is_admin) {
        return <div className={styles.state}>No tienes permisos para acceder aquÃ­.</div>;
    }
    ...
    return (
        <div className={styles.container}>
            ...
            <main className={styles.content}>
                {loading ? (
                    <div className={styles.state}>Cargando cola de moderaciÃ³n...</div>
                ) : queue.length === 0 ? (
                    <div className={styles.state}>No hay elementos que requieran atenciÃ³n en esta categorÃ­a.</div>
                ) : (
                        ...
                        {queue.map(item => (
                            <div key={item.id} className={styles.card}>
                                ...
                                <div className={styles.aiReason}>
                                    ...
                                    <p className={styles.reasonText}>"{item.ai_reason || 'Sin razÃ³n especificada'}"</p>
                                </div>
                                ...
                                <div className={styles.userReasons}>
                                    ...
                                    <ul className={styles.reasonsList}>
                                        {item.reports?.map((r, idx) => (
                                            <li key={r.id || idx}>
                                                <strong>{r.reporter?.display_name || 'AnÃ³nimo'}:</strong> {r.reason}
                                            </li>
                                        ))}
                                    </ul>
                                </div>
                                ...
                                {item.status === 'appealed' && (
                                    <div className={styles.userAppeal}>
                                        <div className={styles.reasonTitle}>RazÃ³n de la apelaciÃ³n:</div>
                                ...
                                <div className={styles.actions}>
                                    {item.status !== 'resolved' ? (
                                        ...
                                        <div className={`${styles.resolutionBadge} ${item.post?.is_banned ? styles.banBadgeResult : styles.restoreBadgeResult}`}>
                                            {item.post?.is_banned ? (
                                                <>
                                                    <XCircle size={16} /> AcciÃ³n: Baneo Confirmado
                                                </>
                                            ) : (
                                                <>
                                                    <CheckCircle size={16} /> AcciÃ³n: PublicaciÃ³n Restaurada
                                                </>
                                            )}
                                        </div>
                                    )}
