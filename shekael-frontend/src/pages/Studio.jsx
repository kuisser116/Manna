...
export default function Studio() {
    ...
    const fetchStats = async () => {
        try {
            ...
            showError('Error', 'No se pudieron cargar tus estadÃ­sticas');
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmAppeal = async (reason) => {
        ...
        try {
            ...
            if (res.ok) {
                showSuccess('ApelaciÃ³n enviada', 'Revisaremos tu caso lo antes posible.', true);
                fetchStats();
            } else {
                const errData = await res.json();
                showError('Error', errData.message || 'No se pudo enviar la apelaciÃ³n');
            }
        } catch (error) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            showSuccess('Eliminado', 'Tu publicaciÃ³n ha sido borrada.', true);
            setPosts(prev => prev.filter(p => p.id !== selectedPost.id));
            // Opcional: Recalcular estadÃ­sticas locales
        } catch (error) {
            showError('Error', 'No se pudo eliminar la publicaciÃ³n');
        }
    };
    ...
    const getPostTitle = (post) => {
        ...
        const parts = (post.content || '').split('|||');
        if (post.type === 'video') return parts[1] || 'Video sin tÃ­tulo';
        if (post.type === 'image') return parts[2] || parts[1] || 'Imagen de Shekael';
    ...
    return (
        <div className={styles.portalWrapper}>
            <header className={styles.portalHeader}>
                <div className={styles.headerLeft}>
                    <Link to="/feed" className={styles.backBtn}>
                        <ArrowLeft size={20} />
                        <span>Volver a Shekael</span>
                    </Link>
                    <div className={styles.divider} />
                    <div className={styles.brand}>
                        <img src={logoImg} alt="Shekael" className={styles.headerLogo} />
            ...
            <div className={styles.container}>
                <header className={styles.pageHeader}>
                    <div className={styles.titleArea}>
                        ...
                        <div>
                            <h1>EstadÃ­sticas de Creador</h1>
                            ...
                            posts.map(post => (
                                <div key={post.id} className={styles.postCard}>
                                        ...
                                        <div className={styles.postThumbWrapper}>
                                            {getThumbnail(post) ? (
                                                ...
                                                <div className={styles.postThumbFallback}>
                                                    {post.type === 'video' ? 'ðŸŽ¥' : post.type === 'image' ? 'ðŸ–¼ï¸' : 'ðŸ“'}
                                                </div>
                                            )}
                                    ...
                                    <div className={styles.postActions}>
                                        <Link to={`/post/${post.id}`} className={styles.actionBtn} title="Ver publicaciÃ³n">
            ...
            <ConfirmationModal 
                ...
                message={`EstÃ¡s a punto de eliminar "${getPostTitle(selectedPost)}". Esta acciÃ³n borrarÃ¡ todas las estadÃ­sticas y archivos asociados permanentemente.`}
