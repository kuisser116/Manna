...
const POST_TYPES = [
    { value: 'micro-text', label: 'Texto', icon: Type, desc: 'Un pensamiento (â‰¤280 chars)' },
    { value: 'image', label: 'Imagen', icon: Image, desc: 'Subida a IPFS â€” permanente' },
    { value: 'video', label: 'Video', icon: Video, desc: 'Streaming Web3 via Livepeer' },
    { value: 'capsule', label: 'CÃ¡psula âœ¨', icon: Sparkles, desc: 'ReflexiÃ³n destacada' },
];
...
export default function CreatePost() {
    ...
    // â”€â”€ Video: wizard callback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    const handleVideoPublish = async ({ videoFile, thumbnailFile, title, description, tags, visibility, scheduledAt }) => {
        ...
        showLoading('Validando contenido...', 'La IA estÃ¡ revisando tu video antes de iniciar la subida');
        ...
        try {
            ...
            const response = await new Promise((resolve, reject) => {
                ...
                const simulateProgress = () => {
                    ...
                    const estimatedDuration = Math.max(10, totalSizeMB * 2); // 2s por MB, mÃ­nimo 10s
                ...
                xhr.upload.addEventListener('progress', (e) => {
                    if (e.lengthComputable) {
                        ...
                        // Si es 100% desde el principio, ignorar y dejar que la simulaciÃ³n funcione
                ...
                // TambiÃ©n escuchar el progreso general (no solo upload)
                ...
                // Inicializar con 0% despuÃ©s de un breve retraso
                setTimeout(() => {
                    if (!progressStarted) {
                        setModalState(prev => ({
                            ...prev,
    ...
    const handleSubmit = async (e) => {
        ...
        if (type === 'image') {
            ...
            showLoading('Validando imagen...', 'La IA estÃ¡ revisando tu contenido...');
            try {
                // Pre-validar imagen
                ...
                showSuccess('Â¡Imagen publicada en IPFS!', result.cid ? `CID: ${cid}...` : 'âš ï¸ Modo demo', true);
                setTimeout(() => navigate('/feed'), 2000);
            } catch (err) {
                hideModal();
            ...
            showLoading('Validando post...', 'La IA estÃ¡ revisando tu mensaje...');
            try {
                const aiCheck = await preValidateContent({
                    text: content,
                ...
                showSuccess('Â¡Publicado!', 'Tu post ya estÃ¡ en el feed', true);
                setTimeout(() => navigate('/feed'), 1400);
            } catch (err) {
                hideModal();
                const errorMsg = err.response?.data?.reason || err.response?.data?.message || err.message || 'IntÃ©ntalo de nuevo';
        ...
        <div className={styles.layout} style={{ '--pattern-url': `url(${bgPatternUrl})` }}>
            <main className={styles.main}>
                <div className={styles.header}>
                    ...
                    <p className={styles.subtitle}>Publica algo que valga la pena. AquÃ­ el contenido importa.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className={styles.form}>
                        ...
                        <div className={styles.imageZone}>
                                ...
                                <div className={styles.imagePreviewWrap}>
                                    ...
                                    <AnimatePresence>
                                        {compressionStats && (
                                            ...
                                            >
                                                <Zap size={12} />
                                                {(originalSize / 1024).toFixed(0)}KB â†’ {compressionStats.compressedKB}KB
                            ...
                            {!imagePreview && (
                                <label className={styles.uploadLabel}>
                                    ...
                                    <span className={styles.uploadSub}>Se comprime automÃ¡ticamente Â· LÃ­mite 5MB</span>
                    ...
                    {/* Textarea texto/cÃ¡psula */}
                    {(type !== 'image' && type !== 'video') && (
                        <div className={styles.textareaWrap}>
                            <textarea
                                className={`${styles.textarea} ${isCapsule ? styles.capsuleTextarea : ''}`}
                                placeholder={isCapsule ? 'Â¿QuÃ© reflexiÃ³n quieres dejarles hoy?' : 'Â¿QuÃ© tienes en la cabeza? Dilo sin filtros.'}
                                value={content}
                                onChange={(e) => setContent(e.target.value)}
                                rows={isCapsule ? 4 : 6}
                                maxLength={maxLength}
                    ...
                    {type !== 'video' && (
                        <div className={styles.depositNote}>
                            ðŸ”’ DepÃ³sito de Confianza: 0.10 XLM se bloquea al publicar y se devuelve en 24h
                        </div>
                    )}
                    ...
                    {type !== 'video' && (
                        ...
                        >
                            {compressing ? 'â³ Comprimiendo...' : 'Publicar'}
                        </button>
                    )}
                </form>
